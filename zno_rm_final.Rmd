...
* variable target: biaya medis
* variable prediktor: jenis kelamin, umur, riwayat merokok
...
* variable target: harga mobil
* variable prediktor: tahun pembuatan, torsi mesin, jumlah silinder
...
* Sales dan profit memiliki korelasi sangat kuat dan positif. Semakin besar sales, maka profit juga semakin besar.
* Discount dan profit memiliki korelasi sedang dan negatif. Semakin besar discount, maka profit semakin kecil.

> Kesimpulan: Prediktor sales, quantity, dan discount tetap digunakan sebagai prediktor.
...
💡 Insight: 
* Berdasarkan plot, semakin besar sales (total harga) maka semakin tinggi juga profit yang didapatkan oleh perusahaan.
...
model_ols <- lm(formula = Profit ~ Sales,
                data = copiers)
...
> Arti: Ketika Sales = 0, maka Profit bernilai -114. -> Rugi sebesar -114.
...
* Dari Intercept: -114. Artinya, ketika Sales bernilai 0, maka kita rugi sebesar 114.

* Dari Slope: 0.42286. Artinya Setiap peningkatan 1 unit dalam Sales akan meningkatkan Profit sebesar 0.42286 unit, dengan asumsi faktor lain tetap konstan.

* Dari P-value: Maka prediktor Sales merupakan prediktor yang signifikan. Ditandai dengan jumlah bintang sebanyak 3.

* Dari R-squared: 0.8828 artinya model kita cukup baik karena nilai multiple r-squared nya mendekati 1.
...
- [X] Variable target harus numerik -> regresi
- [ ] Variable target harus kategorik -> klasifikasi
...
- [ ] Nilai intercept dari garis tersebut adalah -20 -> 10
- [ ] Ketika nilai B naik satu satuan, maka nilai A akan naik sebesar 20 satuan  -> turun 20 satuan
- [X] Hubungan antara variable A dan B adalah negatif
- [ ] B adalah prediktor yang signifikan untuk memprediksi nilai A. -> tidak tahu. harus cek summary model (banyaknya bintang pada prediktor B)

4. R-squared adalah salah satu nilai untuk menentukan kebaikan model regresi. R-squared yang diharapkan adalah nilai yang mendekati ...

- [ ] nol
- [X] satu -> rentang nilai R-Squared adalah 0 s.d. 1
- [ ] tak terhingga
...
💡Insight: Karena garis regresi antara model_ols dan model_no_outlier berhimpit, maka outlier nya termasuk kategori low influence.
...
💡Insight:
- R-Squared model_ols = 0.883
- R-Squared model_no_outlier = 0.851

**Kesimpulan**: Model dengan outlier memiliki nilai R-Sqaured yang lebih baik. Maka, kita pertahankan nilai outlier tersebut.
...
model_ols_multi <- lm(formula = Profit ~ .,
                      data = copiers)
...
1. Interpretasi coefficient untuk prediktor numerik:
  + Ketika Sales naik satu satuan, maka Profit akan naik sebesar 0.42124, dengan catatan nilai prediktor lainnya tetap.
  + Ketika Quantity naik satu satuan, maka Profit akan turun sebesar 13.40975, dengan catatan nilai prediktor lainnya tetap.
  + Ketika Discount naik satu satuan, maka Profit akan turun sebesar 874.83099, dengan catatan nilai prediktor lainnya tetap.
...
Adjusted R-squared: 0.9442254

```{r}
compare_performance(model_ols, model_ols_multi)
```
Kesimpulan:
- Model dengan banyak prediktor (model_ols_multi) menghasilkan nilai adj. r-squared yang lebih baik. Hal ini menandakan bahwa model mampu menangkap pola pada data dengan lebih baik dibandingkan dengan menggunakan 1 prediktor.
...
Kesimpulan: Prediktor yang signifikan terhadap target adalah sales dan discount ditandai dengan adanya 3 bintang pada prediktor tersebut (signif. codes).
...
model_ols_multi2 <- lm(formula = Profit ~ Sales + Discount, 
                       data = copiers)
...
- model_ols_multi: 0.9442 -> menggunakan semua prediktor
- model_ols_multi2: 0.9462 -> menggunakan 2 prediktor signifikan

> Kesimpulan: Berdasarkan nilai adj. r-squared, model yang menggunakan 2 prediktor signifikan hasilnya tidak jauh berbeda dengan model yang menggunakan semua prediktor. Bahkan lebih baik.
...
- [ ] Linear regression dengan satu prediktor -> simple linear regression
- [X] Linear regression dengan prediktor lebih dari satu -> multiple linear regression
...
- [X] Hanya boleh menggunakan prediktor yang berkorelasi sangat kuat saja
- [ ] Variable prediktor kategorik dengan jumlah sama yang banyak sebaiknya tidak digunakan
...
> Formula: (n-1) di mana n adalah banyaknya kategori pada prediktor tersebut
...
Manakah model terbaik? Model A dan Model B.
...
💡 Insight: Artinya, model kita memprediksi dengan tingkat penyelewengan sebesar 73.47894.
...
> Interpretasi RMSE: Secara rata-rata nilai prediksi model menyimpang sejauh 94.97767 dari nilai aktualnya. Baik ke atas maupun ke bawah.
...
> Interpretasi MAPE: Hasil prediksi menyimpang sejauh 29% dari nilai aktualnya.
...
> Model terbaik berdasarkan MAPE adalah model_ols_multi (menggunakan semua prediktor)
...
💡 Insight:  Semua kolom sudah memiliki tipe data yang sesuai. Sehingga tidak perlu dilakukan perubahan tipe data.
...
- Korelasi kuat terhadap target (balance) -> rating, limit, income
- Korelasi rendah terhadap target (balance) -> education, age, cards
- Terdapat korelasi sangat kuat antara prediktor limit dan rating.
...
- Prediktor yang signifikan:
  Jawaban: Income, Rating, Cards, Age, Student
  
- Nilai yang dipakai adalah [Adjusted R-Squared]? Berapa nilainya?
  Jawaban: 0.9279 
...
💡 Insight : Berdasarkan nilai MAPE, nilai prediksi menyimpang sejauh 26.9% dari aktualnya.
...
💡 Insight: Berdasarkan nilai adj. r squared, ternyata semua model menghasilkan skor yang sama.
...
**Kesimpulan**: Karena nilai residual berada di sekitar angka 0, maka uji asumsi linearity TERPENUHI.

**Status**:
Uji asumsi Linearity [`TERPENUHI`].
...
**Status**:
Uji asumsi Normality of Residuals [`TIDAK TERPENUHI`].
...
Nilai p-value = 0.00000000001243 < 0.05
...
**Status**:
Uji asumsi Homoscedasticity of Residuals [`TIDAK TERPENUHI`].
...
```
**Kesimpulan**: 
Tidaka ada prediktor yang memiliki skor VIF > 10. Artinya, model backward tidak terjadi Multicollinearity.

**Status**:
Uji asumsi No Multicollinearity [`TERPENUHI`].
...
2. Normality of Residuals -> [`TIDAK TERPENUHI`]
3. Homoscedasticity of Residuals -> [`TIDAK TERPENUHI`]
4. No Multicollinearity -> [`TERPENUHI`]

> Berdasarkan hasil uji asumsi, model backward hanya memenuhi 2 asumsi yaitu linearity dan no milticolinearity. Tetapi, dari 4 asumsi, yang wajib terpenuhi adalah linearity dan no milticolinearity. Jadi model backward bisa digunakan.
